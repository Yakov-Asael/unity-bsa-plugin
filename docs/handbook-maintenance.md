...
The `handbook-refresh` skill sorts claims into three tiers, and the distinction is the whole point of the policy:
...
2. Run the `handbook-refresh` skill for the tabs in scope. **It is read-only against Salesforce** and proposes edits rather than applying them.
